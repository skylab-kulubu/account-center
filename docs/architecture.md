...
Sudo token Keycloak’ın verdiği opak bir JWT’dir; tarayıcıya asla verilmez. `SudoVault` (`src/server/auth/sudo.ts`) onu oturum kaydına bağlı AAD (`session:<id>:sudo`) ile AES-256-GCM şifreleyerek `account_sessions.sudo_token_ciphertext` (diğer envelope sütunları gibi `text`) ve `sudo_expires_at` sütunlarında tutar (migration `0006_account_sudo.sql`; sütunlar birlikte boş ya da dolu olmak zorundadır). `requireFreshSudo(sessionId)` yalnız aktif, iptal edilmemiş oturumun beş saniyeden fazla ömrü kalan proof’unu düz metin olarak döndürür; yoksa ya da süresi dolduysa kaydı temizler ve mevcut yöntem ipucuyla (`password`/`totp`/`passkey`) `SudoRequiredError` fırlatır. Yerel ömür en fazla 15 dakikayla sınırlanır; SPI beş dakikalık token verir. Şifresi çözülemeyen materyal atılır ve yeniden doğrulama istenir. Süresi dolmuş proof’lar saatlik prune işinde aktif oturumlardan silinir; oturum iptali/silinmesi materyali beraberinde götürür.
