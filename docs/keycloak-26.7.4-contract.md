...
- `OIDC_UPSTREAM_SESSION_MAX_SECONDS`, doğrulanmış Keycloak SSO Session Max değerinden config-as-code ile beslenmeli. BFF absolute deadline’ı `min(callback now + 8 saat local cap, Keycloak oturumunun bitişi)` olarak uygular. Keycloak oturumunun bitişi imzalı ID token'daki `sky_session_expires` claim'idir (Keycloak'ın bu SSO oturumunu max lifespan ile kapatacağı an, epoch saniye; remember-me ve client override'ları dahil; `account-center` üzerindeki SKY LAB protocol mapper'ı yazar). Claim `auth_time`'dan sonra ve en fazla 31 gün ileride olmalıdır; geçmişte kalmışsa oturum bitmiştir ve giriş reddedilir. Claim yoksa ya da geçersizse bitiş `upstream session başlangıcı + OIDC_UPSTREAM_SESSION_MAX_SECONDS` olarak tahmin edilir (önceki davranış): başlangıç native handoff'ta callback anıdır (köprü yeni bir web session açar ama uygulamanın özgün `auth_time` değerini taşır); diğer girişlerde `sky_session_started` claim'idir (Keycloak user session'ının `started` anı), claim yoksa, güvenli bir tam sayı değilse ya da `auth_time − 5 sn` ile `şimdi + 5 sn` aralığının dışındaysa doğrulanmış `auth_time` kullanılır. Canlı bir Keycloak oturumunda `prompt=login` ile yeniden doğrulanan kişide `started` eski kalır, `auth_time` ilerler; bu tahmin yolunda claim yok sayılır ve BFF oturumu Keycloak oturumundan biraz uzun görünebilir (Keycloak oturumu bitince refresh reddedilir). `sky_session_expires` taşıyan girişlerde bu durum ve remember-me oturumlarının 8 saat sonra her girişte `upstream_session_expired` alması ortadan kalkar. Mevcut ama bozuk bir `sky_session_*` claim'i (yanlış tür, kesirli, aralık dışı) atılır ve giriş başına bir kez, değeri yazılmadan `oidc_session_claims` / `session_claim_ignored` olarak loglanır. Web handoff (ADR-0048) yeni bir Keycloak session açıp uygulamanın özgün `auth_time` değerini taşıdığı için, uygulamadaki girişi 8 saatten eski olan kişinin `my.` oturumu bu claim'ler sayesinde handoff anına göre hesaplanır. `auth_time` değişmez: Sudo modu ve hesap silme taze giriş kanıtını yine ayrı bir `prompt=login&max_age=0` turundan ister. Süresi geçmiş upstream session `oidc_login_failed` / `upstream_session_expired` olarak loglanır ve placeholder/güvensiz aralığı startup’ta reddeder.
- ID token'daki `sky_embed` (user session note `sky.embed` mapper'ı) tam olarak `"skyapp"` ise ya da giriş native handoff'tan geliyorsa callback, BFF session'ıyla aynı ömürde yalnız görünüm için `__Host-sky-account-embed=skyapp` çerezini yazar; hesap sayfaları bu çerez varken mobil üst çubuğu çizmez, çünkü SkyApp WebView'ının kendi header'ı vardır. Hiçbir yetki bu çereze ya da `sky_session_*` claim'lerine bağlı değildir; `sky_embed` yoksa ya da başka bir değer taşıyorsa çerez yazılmaz.
